...
final internal class RealLocalDataSource: LocalDataSource {
  ...
  private let realmDispatchQueue = DispatchQueue(
    label: "com.downloaderKit.realm.serial-queue",
    qos: .userInitiated
  )
  ...
  private func getRefreshedRealmAdapter() throws -> RealmAdapter {
    ...
    print("[DownloaderKit] [DEBUG] getRefreshedRealmAdapter refreshResult=\(refreshResult)")
      ...
      let item = DispatchWorkItem {
        ...
        autoreleasepool {
          do {
            ...
            let refreshResult = realm.refresh()
            print("[DownloaderKit] [DEBUG] useRealmAdapter refreshResult=\(refreshResult)")
            if disposable.isDisposed { return }
  ...
  func update(id: String, state: DownloadKitState) -> Completable {
    useRealmAdapter { realm, checkDisposed in
      ...
      guard task.canTransition(to: state) else {
        print("[DownloaderKit] [DEBUG] cannot transition from \(task.downloadState) to \(state)")
        return
      }
      
      if state == .cancelled, !task.canCancel {
        print("[DownloaderKit] [DEBUG] cannot cancel \(id), task.state=\(task.downloadState)")
        throw DownloaderKitError.cannotCancel(identifier: id)
      }
...
extension RealmAdapter {
  ...
  fileprivate func findDownloadTaskEntity(by id: String) throws -> DownloadTaskEntity {
    ...
    throw DownloaderKitError.notFound(identifier: id)
  }
}
